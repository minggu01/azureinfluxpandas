...
def read_df_from_azure(azure_data_store, file_path, token):
    ...
    try:
        ...
        with adls_file_system_client.open(file_path, 'rb') as f:
            ...
            df = pd.read_csv(f, sep=";", header=None, names=["Sensor", "Value", "Timestamp", "Status"],
...
def walk_and_tag_azure(base_path, azure_data_store, token, include_regexp=".*", exclude_regexp="a^"):
    ...
    for file_path in walk_azure(base_path, azure_data_store, token, include_regexp, exclude_regexp):
        tag = os.path.basename(os.path.dirname(file_path))
        yield file_path, tag


def walk_azure(base_path, azure_data_store, token, include_regexp=".*", exclude_regexp="a^"):
    ...
    logger.debug("Starting a azure walk with base_path: {} on data_store {} with include_regexp: {} "
                 "and exclude_regexp: {}".format(base_path, azure_data_store, include_regexp, exclude_regexp))
    ...
    while fringe:
        ...
        if a_path['type'] == 'FILE':
            ...
            if include_regexp.match(file_path) and not exclude_regexp.match(file_path):
                ...
                logger.debug("Found that the file_path %s does not satisfy regexp requirements" % file_path)